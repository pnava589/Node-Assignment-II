...
const SingleFavorite=(props)=>{
    const [favorite,setFavorite] = useState(props.data);
    ...
    /*useEffect(()=>{
        const favorites = fetch('http://localhost/api/favorites')
        .then(resp => resp.json())
        .then(data => setFavorites(resp.forEach(item => favorites.push(item))))
    });*/
    return(
